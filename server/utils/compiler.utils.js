...
exports.getCompileCommand = (
  ...
  switch (language.toLowerCase()) {
    ...
    case "java":
      ...
      const classDir = path.dirname(outputFilePath);
...
exports.getExecuteCommand = (
  ...
  switch (language.toLowerCase()) {
    ...
    case "cpp":
      ...
      break;
    case "java":
      // Extract class name from path using path.basename for cross-platform compatibility
      const path = require("path");
  ...
  if (input && input.trim() !== "") {
    ...
    // Use different echo syntax based on platform (Windows vs Linux/Mac)
    const os = require("os");
    const isWindows = os.platform() === "win32";
    return isWindows
      ? `echo "${escapedInput}" | ${command}`
      : `echo "${escapedInput}" | ${command}`;
  }
